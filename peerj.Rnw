...
tab2 <- transform(tab2, USWest = round(tab2$USWest,1)) # round off to two decimals
...
for (i in 1:12) {
  tab2$sig[[i]] <- stars.pval(calculateFishersP(i, d.temp.h1, d.temp.h0))
  tab2$USWestOR[[i]] <- round(calculateFishersOR(i, d.temp.h1, d.temp.h0),1)
}

# Print
tab2$sig <- gsub("\\s","",tab2$sig) # remove whitespace
tab2$sig <- gsub("\\.","+",tab2$sig) # change . (p<0.10) to +
tab2$USWest <- paste(format(tab2$USWest,digits=2,nsmall=1), tab2$sig, sep="")
tab2$sig <- NULL
# -------- END Make USWest
...
for (i in 1:12) {
  tab2$sig[[i]] <- stars.pval(calculateFishersP(i, d.temp.h1, d.temp.h0))
  tab2$USEastOR[[i]] <- round(calculateFishersOR(i, d.temp.h1, d.temp.h0),1)
}

# Print
tab2$sig <- gsub("\\s","",tab2$sig) # remove whitespace
tab2$sig <- gsub("\\.","+",tab2$sig) # change . (p<0.10) to +
tab2$USEast <- paste(format(tab2$USEast,digits=2,nsmall=1), tab2$sig, sep="")
tab2$sig <- NULL
# -------- END Make USEast
...
for (i in 1:12) {
  tab2$sig[[i]] <- stars.pval(calculateFishersP(i, d.temp.h1, d.temp.h0))
  tab2$LondonOR[[i]] <- round(calculateFishersOR(i, d.temp.h1, d.temp.h0),1)
}

# Print
tab2$sig <- gsub("\\s","",tab2$sig) # remove whitespace
tab2$sig <- gsub("\\.","+",tab2$sig) # change . (p<0.10) to +
tab2$London <- paste(format(tab2$London,digits=2,nsmall=1), tab2$sig, sep="")
tab2$sig <- NULL
# -------- END Make London
...
for (i in 1:12) {
  tab2$sig[[i]] <- stars.pval(calculateFishersP(i, d.temp.h1, d.temp.h0))
  tab2$WorldRestOR[[i]] <- round(calculateFishersOR(i, d.temp.h1, d.temp.h0),1)
}

# Print
tab2$sig <- gsub("\\s","",tab2$sig) # remove whitespace
tab2$sig <- gsub("\\.","+",tab2$sig) # change . (p<0.10) to +
tab2$WorldRest <- paste(format(tab2$WorldRest,digits=2,nsmall=1), tab2$sig, sep="")
tab2$sig <- NULL
# -------- END Make WorldRest
...
print(xtable(caption="Descriptives using xtable", tab2[,-c(6:14)], digits=c(0,0,0,0,0,0), align=c("l", "l", "r","r","r","r")), caption.placement = "top")
@
...
xtable.val2 <- xtable(tab2.temp.matrix)
...
addtorow$command <- paste0("& ", # fix first column element
                           paste0('& \\multicolumn{2}{c}{', c("USWest", "USEast", "UK South", "WorldRest"), '}', collapse=''), '\\\\')
 
print(xtable.val2, add.to.row=addtorow, include.colnames=F, hline.after = c(-1,1,nrow(xtable.val2)))
@
